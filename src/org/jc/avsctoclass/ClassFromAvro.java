...
import org.jc.classbuilder.builder.ClassBuilder;
import org.jc.classbuilder.builder.MethodsDescription;
...
public class ClassFromAvro extends ClassBuilder<Schema, Schema.Field>{
    ...
    protected void addMethods(CtField field, CtClass fullClass) throws Exception {
        ...
        try {
            ...
            MethodsDescription.addNewMethod(fullClass.getName(), "public void set" + methodName +"(" + field.getType().getName() + " " + field.getName() + "){ this." + field.getName() + " = " + field.getName() + ";}");
            /*
            ...
            CtMethod setter = CtNewMethod
                    .make("public void set" + methodName +"(" + field.getType().getName() + " " + field.getName() + "){ this." + field.getName() + " = $1;}",// + $1 + ";}", 
                            fullClass);*/

            CtMethod getter = CtNewMethod.getter("get" + methodName, field);
            getter.setModifiers(Modifier.PUBLIC);
            
            fullClass.addMethod(getter);
            
            CtMethod setter = CtNewMethod.setter("set" + methodName, field);
            //setter.addParameter(field.getType());
